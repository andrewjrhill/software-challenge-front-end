...
const ScanUpdateModal = props => {
    const { users, scan, active, onSubmit, onCloseUpdateScanModal, onDelete } = props;
    ...
    const formData = {
        ...
        id: parseInt(id),
        imageURL: scan.imageURL ? scan.imageURL : `thumbnails/scan-0${Math.floor(Math.random() * 10)}.png`,
        name,
    ...
    return (
        <div className={ classes }>
            <form>
                ...
                <button className="close-button" onClick={ e => { e.preventDefault(); onCloseUpdateScanModal(); } }><XCircle /></button>
                ...
                <label>
                    Scanned By:
                    <select value={ scannedByUserId }
                            onChange={e => {
                                setScannedByUserId(Number(e.target.value));
                                setUserName(users.find(user => user.id === Number(e.target.value)).name);
                            }}>
                ...
                <div className="button-wrapper">
                    <button className="button-primary" onClick={ e => { e.preventDefault(); onSubmit(formData); } }>Submit</button>
