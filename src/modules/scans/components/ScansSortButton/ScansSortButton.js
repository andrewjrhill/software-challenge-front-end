...
const ScansSortButton = ({ onClick, property, direction, currentSort }) => {
    ...
    return (
        <button className={ classes } onClick={ () => { onClick(property, direction) }}>
            { direction && direction === 'asc' ? <span>Ascending <ChevronsUp /></span> :
                           direction === 'desc' ? <span>Descending <ChevronsDown /></span> : null }
        </button>
    );
}
